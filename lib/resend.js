...
export async function sendConfirmation({ booking, client, zoomLink, sessionsTotal = 1, paymentPlan = false, instalmentCents = 0, totalCents = 0, recurDates = null }) {
  ...
  // Build ICS attachments — one per session date for recurring bookings
  const allDates = recurDates && recurDates.length > 1 ? recurDates : [booking.date]
  const clientAttachments = allDates.map((dateStr, i) => {
    const sessionIcs = buildICS({
      summary: `${booking.session_type_name} with Vik — Vikashan`,
      description: `Join at: ${zoomLink}\n\nNeed to reschedule? Reply to your confirmation email.`,
      location: zoomLink,
      dateStr,
    ...
    const label = allDates.length > 1 ? `vikashan-session-${i + 1}.ics` : 'vikashan-session.ics'
    return { filename: label, content: Buffer.from(sessionIcs).toString('base64'), type: 'text/calendar; method=REQUEST' }
  })
  ...
  const html = emailWrapper({
    ...
    footer: `
      ...
      <p style="font-size:12px;color:#aaa;margin-top:8px">${allDates.length > 1 ? `${allDates.length} Outlook / Apple Calendar invites are attached to this email.` : 'An Outlook / Apple Calendar invite is attached to this email.'}</p>
  ...
  // Admin gets ICS for every session date
  const adminAttachments = allDates.map((dateStr, i) => {
    ...
    const label = allDates.length > 1 ? `vikashan-session-${i + 1}.ics` : 'vikashan-session.ics'
    return { filename: label, content: Buffer.from(adminIcs).toString('base64'), type: 'text/calendar; method=REQUEST' }
  })
  ...
  await Promise.all([
    ...
    resend.emails.send({
      ...
      html: emailWrapper({
        ...
        footer: `<p style="font-size:12px;color:#aaa;margin-top:8px">${allDates.length > 1 ? `${allDates.length} calendar invites attached.` : 'Calendar invite attached.'}</p>`
      }),
