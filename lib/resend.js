...
export async function sendConfirmation({ booking, client, zoomLink, sessionsTotal = 1 }) {
  ...
  const html = emailWrapper({
    ...
    details: {
      ...
      'Amount': booking.amount_cents === 0 ? 'Free' : sessionsTotal > 1 ? `$${(booking.amount_cents / 100).toFixed(0)} × ${sessionsTotal} sessions = $${(booking.amount_cents * sessionsTotal / 100).toFixed(0)}` : `$${(booking.amount_cents / 100).toFixed(0)}`,
    },
